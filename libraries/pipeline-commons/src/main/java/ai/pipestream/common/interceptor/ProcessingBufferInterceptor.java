...
public class ProcessingBufferInterceptor {
    ...
    private void captureFromProcessResponse(Object result, ProcessingBuffer<PipeDoc> buffer) {
        if (result instanceof ai.pipestream.data.module.ModuleProcessResponse) {
            ai.pipestream.data.module.ModuleProcessResponse response = (ai.pipestream.data.module.ModuleProcessResponse) result;
            if (response != null && response.hasOutputDoc()) {
