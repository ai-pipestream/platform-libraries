...
/**
 * Quarkus build-time processor for the Pipeline Protobuf Kafka Connector
 * extension.
 *
 * <p>
 * This processor implements the "No-Cheating" architecture that automatically
 * configures
 * Kafka channels for Protobuf message serialization/deserialization. Instead of
 * requiring
 * manual configuration in application.properties, it scans the codebase at
 * build time
 * to discover {@link ai.pipestream.api.annotation.ProtobufIncoming} and
 * {@link ai.pipestream.api.annotation.ProtobufChannel} annotations and
 * generates
 * the appropriate SmallRye Reactive Messaging configuration.
 * </p>
 *
 ...
 * </p>
 * <ol>
 * <li><b>Build-Time Discovery:</b> Uses Jandex to scan for Protobuf annotations
 * and
 * extract message types</li>
 * <li><b>Configuration Generation:</b> Produces
 * {@link SystemPropertyBuildItem}s that
 * configure Kafka serializers, deserializers, and Apicurio Registry
 * settings</li>
 * </ol>
 *
 * <h2>Key Features</h2>
 * <ul>
 * <li>Automatic Protobuf type detection from method parameters and field
 * generics</li>
 * <li>Topic name derivation from channel names (removes
 * -producer/-consumer/-in/-out suffixes)</li>
 ...
 * <h2>Thread Safety</h2>
 * <p>
 * This processor is designed to be thread-safe as it only reads from the Jandex
 * index
 * and produces build items. No mutable state is maintained between build steps.
 * </p>
 *
...
public class ProtobufKafkaProcessor {
    ...
    /**
     * DotName for the {@link ai.pipestream.api.annotation.ProtobufIncoming}
     * annotation used on consumer methods.
     */
    private static final DotName PROTOBUF_INCOMING = DotName
            .createSimple("ai.pipestream.api.annotation.ProtobufIncoming");

    /**
     * DotName for the {@link ai.pipestream.api.annotation.ProtobufChannel}
     * annotation used on producer fields.
     */
    private static final DotName PROTOBUF_CHANNEL = DotName
            .createSimple("ai.pipestream.api.annotation.ProtobufChannel");
     ...
     *
     * <p>
     * This feature name appears in the Quarkus startup banner and build logs,
     * indicating that the Pipeline Protobuf Kafka Connector extension is active.
     * </p>
     *
     ...
     *
     * <p>
     * This method implements the "No-Cheating" architecture by scanning the Jandex
     * index
     * at build time to discover Protobuf message consumers and producers, then
     * generating
     * the appropriate SmallRye Reactive Messaging configuration properties.
     * </p>
     *
     * <h3>Processing Flow</h3>
     * <ol>
     * <li>Configures global Apicurio Registry URL bridging</li>
     * <li>Scans for {@link ai.pipestream.api.annotation.ProtobufIncoming}
     * annotations on methods</li>
     * <li>Scans for {@link ai.pipestream.api.annotation.ProtobufChannel}
     * annotations on fields</li>
     * <li>Generates configuration for each discovered channel</li>
     * </ol>
     *
     * <h3>Configuration Generated</h3>
     * <p>
     * For each consumer/producer channel, this method generates:
     * </p>
     * <ul>
     ...
     * </ul>
     *
     * @param combinedIndex The combined Jandex index containing all application
     *                      classes
     * @param props         Producer for {@link SystemPropertyBuildItem}s that
     *                      configure messaging channels
     * @param beans         Producer for {@link AdditionalBeanBuildItem}s that
     *                      register runtime beans
     *
    ...
    void generateConfig(CombinedIndexBuildItem combinedIndex,
        ...
        // REMOVED per user request for simplicity - relying on System Properties only
        // beans.produce(AdditionalBeanBuildItem.unremovableOf("ai.pipestream.common.interceptor.ApicurioConfigurator"));

        // 2. Bridge Registry URL globally with v3 default
        ...
        for (AnnotationInstance annotation : index.getAnnotations(PROTOBUF_INCOMING)) {
            if (annotation.target().kind() == org.jboss.jandex.AnnotationTarget.Kind.METHOD) {
                ...
                // Prefer returnType from annotation if present (string-based, avoids
                // classloading)
                AnnotationValue returnTypeValue = annotation.value("returnType");
                String typeName = null;

                if (returnTypeValue != null && !returnTypeValue.asString().isEmpty()) {
                    typeName = returnTypeValue.asString();
                    LOG.debugf("📝 [Extension] Using returnType from annotation: %s", typeName);
                } else if (!method.parameterTypes().isEmpty()) {
                    // Fallback to parameter inference
                    typeName = method.parameterTypes().get(0).name().toString();
                    LOG.debugf("📝 [Extension] Inferred type from parameter: %s", typeName);
                }

                if (typeName != null) {
                    configureIncoming(props, channel, typeName, annotation);
                } else {
                    LOG.warnf("⚠️ [Extension] Could not determine return type for channel %s on method %s",
                            channel, method.name());
                }
            }
        }
     ...
     *
     * <p>
     * This method generates all necessary configuration properties for a consumer
     * channel
     * annotated with {@link ai.pipestream.api.annotation.ProtobufIncoming}. It sets
     * up
     * the topic, connector, deserializers, and most importantly, the return class
     * that
     * tells Apicurio how to deserialize the Protobuf message.
     * </p>
     *
     * <h3>Configuration Properties Generated</h3>
     * <ul>
     * <li>{@code mp.messaging.incoming.{channel}.topic} - The Kafka topic name</li>
     * <li>{@code mp.messaging.incoming.{channel}.connector} - Set to
     * "smallrye-kafka"</li>
     * <li>{@code mp.messaging.incoming.{channel}.key.deserializer} - UUID
     * deserializer</li>
     * <li>{@code mp.messaging.incoming.{channel}.value.deserializer} - Protobuf
     * deserializer</li>
     * <li>{@code mp.messaging.incoming.{channel}.apicurio.registry.deserializer.value.return-class}
     * - The Protobuf class (from annotation returnType or inferred from method
     * parameter)</li>
     ...
     * @param typeName   The fully qualified name of the Protobuf message class
     *                   (e.g., "com.example.OrderEvent")
    ...
    private void configureIncoming(BuildProducer<SystemPropertyBuildItem> props,
        ...
        String topic = (topicVal != null && !topicVal.asString().isEmpty()) ? topicVal.asString()
                : deriveTopicName(channel);
        ...
        // Use the original property name that was working
     ...
     *
     * <p>
     * This method generates all necessary configuration properties for a producer
     * channel
     * annotated with {@link ai.pipestream.api.annotation.ProtobufChannel}. It sets
     * up
     * the topic, connector, and serializers for sending Protobuf messages to Kafka.
     * </p>
     *
     * <h3>Configuration Properties Generated</h3>
     * <ul>
     * <li>{@code mp.messaging.outgoing.{channel}.topic} - The Kafka topic name</li>
     * <li>{@code mp.messaging.outgoing.{channel}.connector} - Set to
     * "smallrye-kafka"</li>
     * <li>{@code mp.messaging.outgoing.{channel}.key.serializer} - UUID
     * serializer</li>
     * <li>{@code mp.messaging.outgoing.{channel}.value.serializer} - Protobuf
     * serializer</li>
     * <li>Additional properties from the annotation's {@code properties} array</li>
     * </ul>
     *
     ...
     *
     * <p>
     * This method implements a simple convention for topic naming by removing
     * directional suffixes from channel names. This allows using descriptive
     * channel
     * names like "orders-in" and "orders-out" while mapping them to the same
     * underlying Kafka topic "orders".
     * </p>
     *
     ...
     *
     * <p>
     * <strong>Note:</strong> Suffixes are checked in order from longest to shortest
     * to avoid partial matches (e.g., "-consumer" is checked before "-in" to
     * prevent
     * "orders-consumer" from being incorrectly truncated).
     * </p>
     *
     * @param channel The channel name from the annotation
     * @return The derived topic name, or the original channel name if no derivation
     *         applies
     *
     ...
     *
     * <p>
     * This method parses the {@code properties} array from
     ...
     * <p>
     * Each property string should be in the format "key=value". For example:
     * </p>
     * 
     * <pre>{@code
     ...
     *
     * @param props      The build producer for system property items
     * @param prefix     The configuration prefix (e.g.,
     *                   "mp.messaging.incoming.orders")
    ...
    /**
     * Transforms annotations for @ProtobufChannel fields.
     *
     * <p>
     * For fields, we still use the transformer because it's safer and simpler than
     * generating a delegate bean for injection points.
     * </p>
     */
    @BuildStep
    void transformFieldAnnotations(
            BuildProducer<io.quarkus.arc.deployment.AnnotationsTransformerBuildItem> transformers) {
        transformers.produce(new io.quarkus.arc.deployment.AnnotationsTransformerBuildItem(
                (org.jboss.jandex.AnnotationTransformation) new io.quarkus.arc.processor.AnnotationsTransformer() {
                    @Override
                    ...
                    public void transform(TransformationContext context) {
                        if (context.isField()) {
                            AnnotationInstance protoChannel = context.getTarget().asField()
                                    .annotation(PROTOBUF_CHANNEL);
                            if (protoChannel != null) {
                                String channelName = protoChannel.value().asString();
                                context.transform()
                                        .add(DotName
                                                .createSimple("org.eclipse.microprofile.reactive.messaging.Channel"),
                                                AnnotationValue.createStringValue("value", channelName))
                                        .done();
                                LOG.debugf("✨ [Transformer] Added @Channel(\"%s\") to field %s", channelName,
                                        context.getTarget().asField().name());
                            }
                        }
                    }
                }));
     ...
     *
     * <p>
     * This method analyzes the type of a field annotated with
     * {@link ai.pipestream.api.annotation.ProtobufChannel} to determine the
     * Protobuf message type being emitted.
     * </p>
     *
     ...
     * @return The fully qualified class name of the emitted type, or null if
     *         extraction fails
     *
